...
var Test = kind({
	...
	transparencyChanged: function () {
		...
		for (idx = 0, len = this.collection.length; idx < len; idx++) {
			...
			if (isDataTypeJS){
				item.overlayTransparent = this.transparency;	
			} 
			else {
				item.set('overlayTransparent', this.transparency);	
			}
		}

		if (isDataTypeJS) {
			this.$.gridList.syncChildBindings({all: true, force: true});	
		}
	},
	...
	transparencyTypeChanged: function (sender) {
		this.set('transparency', sender.value);
	},
	itemTypeChanged: function (sender) {
		this.set('itemKind', sender.active.value);
	},
	dataTypeMenuChanged: function (sender) {
		this.set('dataType', sender.active.value);
	},
	selectionTypeChanged: function (sender) {
