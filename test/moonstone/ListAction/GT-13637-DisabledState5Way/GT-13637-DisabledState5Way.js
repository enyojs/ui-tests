...
var
	...
	load = require('../../../load'),
	Test = kind({
		name: 'test.GT-13637-DisabledState5Way',
		kind: Panels,
		pattern: 'activity',
		...
		components: [
			{kind:Panel, headerType: 'medium', title: 'List Actions Sample', headerComponents: [
				...
				{kind:TooltipDecorator, components: [
					...
					{kind: ListActions, proportionalWidth: true, iconSrc: '$lib/moonstone/samples/assets/icon-list.png', listActions: [
				...
				{kind:TooltipDecorator, components: [
					...
					{kind: ListActions, autoCollapse: true, iconSrc: '$lib/moonstone/samples/assets/icon-list.png', listActions: [
		...
		activateHandler: function(inSender, inEvent) {
			if (inEvent && inEvent.action) {
				if (inEvent.originator instanceof SelectableItem) {
			...
			if (inEvent.originator instanceof ListActions) {
